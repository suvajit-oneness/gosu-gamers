...
use Alert;
use Auth;
class GamerTSController extends Controller
{
    public function index() {
        ...
        return view( 'website.tournament.gamer_tournament_schedule.index', compact( 'gamertournamentschedule' ) );
    }
    ...
    public function edit( $id ) {
        $tournamentID = Gamer_tournament_schedule::where( 'gamer_tournament_schedules.id', $id )->select('gamer_tournament_schedules.tournament_id' )->first()->tournament_id;
        ...
        return view( 'website.tournament.gamer_tournament_schedule.edit', compact( 'player2', 'player1', 'tournamentID' ) );
    }
    ...
    public function addSchedule( $id ) {
        ...
        $count = $tours->count();
        if ( $count == 1 ) {
            return view( 'website.tournament.gamer_tournament_schedule.winner', compact( 'tours', 'tournament' ) );
        } elseif ( $count == 0 ) {
            return redirect()->route( 'gamer.tournaments.schedule-details', [$id]);
        } else {
            return view( 'website.tournament.gamer_tournament_schedule.add_schedule', compact( 'count', 'tours', 'tournament' ) );
        }
    }
    ...
    public function tournamentschedule( Request $request ) {
        ...
        for ( $i = 0; $i < $count; $i++ ) {
            if ( $request->player1[$i] == null ) {
                dd('hi Gamer');
                return redirect()->route( 'gamer.tournaments.show', [$id]);
            }
        ...
        return redirect()->route( 'gamer.tournaments.show');
    }
