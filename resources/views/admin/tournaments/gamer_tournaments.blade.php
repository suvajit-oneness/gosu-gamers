...
<div class="content">
   <div class="card">
      ...
      <form action="{{route('assignroom')}}" method="post" accept-charset="utf-8" onSubmit="return gamerSelectLimit();">
         @csrf 
         <div class="row">
            <div class="col-md-6" style="float:left;margin-left:10px;">
               <div class="form-group" >
                  ...
                  <select  name="room" class="form-control">
                     ...
                     @foreach($room as $r)
                     <option value="{{$r->room_code}}" selected="selected">{{$r->room_code}}</option>
                     @endforeach
                     @endif
                  </select>
               </div>
         ...
         <div class="row">
            <div class="col-md-6" style="float:left;margin-left:20px;">
               <div class="form-group" >
                  ...
                  <select name="gamer_id[]" id="gamer_id" >
                     <option value="">--Select Gamer--</option>
                     @foreach($tournamentList as $s_key => $gammer)
                     <option value="{{$gammer->gamer_id}}" selected="selected">{{$gammer->fname}} {{$gammer->lname}}</option>
                     @endforeach
                  </select>
                  @endfor
               </div>
