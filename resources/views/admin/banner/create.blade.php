         ...
         <form method="post" action="{{route('banner.store')}}" enctype="multipart/form-data">
            ...
            <fieldset class="mb-3">
               <table class="table border-bottom border-top">
                  <tbody>
                     <tr>
                        <td width="15%" class="text-right border-right text-uppercase">URL  *</td>
                        <td>
                        ...
                        <td>
                           <select class="form-control" name="partner" id="partner">
                              @foreach($partners AS $key => $label)
                                 <option value="{{ $key }}" {{ old("partner") == $key ? "selected" : "" }}>{{ $label }}</option>
                              @endforeach
                           </select>
                        </td>
                     </tr>
            ...
            <div class="text-left">
               <div class="header-elements">
                  ...
                  <a href="{{route('banner.create')}}" class="btn bg-teal-400 ml-2"><i class="fas fa-redo mr-1"></i> Clear</a>
               </div>
            </div>
         </form>
      </div>
