      ...
      <div class="card-header header-elements-inline">
         ...
         <div class="header-elements">
            ...
            <div class="list-icons">
               <a href="{{route('testimonial.create')}}" class="btn bg-teal-400 list-icons-item text-primary-600 text-uppercase"><i class="fas fa-plus mr-1"></i>
               Add Testimonial </a>
            </div>
            @endcan
         </div>
           ...
           <table id="example1" class="table table-striped datatable-responsive">
            ...
            <tbody>
               ...
               <tr>
                  ...
                  <td>{{strip_tags(html_entity_decode($data->content))}}</td>
                  @can('Testimonial-Edit')
                  <td> 
                     @if($data->is_active == 0)
                     <a class="badge bg-danger" href="{{ URL::to('testimonial/change-status/'.$data->id) }}">Inactive</a>
                     @else
                     <a class="badge bg-success" href="{{ URL::to('testimonial/change-status/'.$data->id) }}">Active</a>
                  </td>
                  @endif  @endcan
                  <td>
                     <div class="list-icons">
                        @can('Testimonial-Read')
                        <a class="badge bg-success" href="{{route('testimonial.show',$data->id)}}">Show</a>
                        @endcan
                        @can('Testimonial-Edit')
                        <a class="badge bg-primary"href="{{ route('testimonial.edit',$data->id) }}">Edit</a>                                            
                        @endcan
